...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.init = function() {
		...
		// Allow dfp
		if ((M.provider == 'adsense' || M.provider == 'adx') && self.config.ads.dfp_available) {
			M.provider = 'dfp';
		}
		...
		switch (M.provider) {
			case 'adx':
				M.publisherId="ca-games-pub-2504503821144226"; // Ad Exchange for Games
				faZepto.getScript("http://imasdk.googleapis.com/js/sdkloader/ima3.js", 
					sdkLoadedCallback, 
					function() {
						self.tracking.trackEvent("AdBlocker event", "AdX", famobi_gameID);
					});
				break;
			case 'adsense':
				M.publisherId="ca-games-pub-8700401253704627"; // AdSense for Games
				faZepto.getScript("http://imasdk.googleapis.com/js/sdkloader/ima3.js", 
					sdkLoadedCallback, 
					function() {
				...
				break;
			case 'dfp':
				M.publisherId="ca-games-pub-2504503821144226"; // AdSense for Games
				faZepto.getScript("http://imasdk.googleapis.com/js/sdkloader/ima3.js", 
					sdkLoadedCallback, 
					function() {
				...
				M.dfp_ad_unit_code = '/37336410/InGameInterstitial//' + self.config.aid;
				if (self.config.ads.dev_share) {
					M.dfp_ad_unit_code += '//' + self.config.ads.dev_share;
				}
				M.dfp_ad_sizes = [300, 250];
				M.dfp_ad_div = 'div-gpt-ad-1425630990345-0';
				M.dfp_custom_params = {
					"a": ""+self.config.aid,
				...
				googletag.cmd.push(function() {
					M.dfp_ad_slot1 = googletag.defineSlot(M.dfp_ad_unit_code, M.dfp_ad_sizes, M.dfp_ad_div).addService(googletag.pubads());
					googletag.pubads().disableInitialLoad();
					googletag.pubads().setTargeting("a",""+self.config.aid);
					googletag.pubads().setTargeting("game",""+window.famobi_gameID);
					googletag.pubads().setTargeting("uuid",""+self.config.uuid);
					//googletag.pubads().set('adsense_channel_ids', self.config.ads.adsense_channels.join('+'));
					googletag.pubads().enableSingleRequest();
					googletag.enableServices();
				});

				faZepto.getScript(('https:' == document.location.protocol ? 'https:' : 'http:') + "//www.googletagservices.com/tag/js/gpt.js", 
					undefined, 
					function() {
						self.tracking.trackEvent("AdBlocker event", "GPT", famobi_gameID);
					});
				*/
				break;
			case 'ad4game':
				M.Ad4GameCounter = 0;
				faZepto.getScript(self.config.assetsPath + "/js/4game_sdk.js", 
					sdkLoadedCallback, 
					function() {
	...
	module.prototype.showAd = function(callback, force){
		...
		switch (M.provider) {
			...
			case 'ad4game':
				return M.showInterstitial(callback, force);
			case 'leadbolt':
				return M.showAppWall(callback, force);
			case 'housead':
	...
	module.prototype.showIMA = function(callback, force){
		if (!M.initAdModal({"returnCallback": callback, "force": force, "iframeAllowed": (M.provider == "dfp")})) {
			return false;
		}
		...
		M.adContainerElement.style.margin = '50px auto 0';
		if (M.provider == "dfp") {
			M.adContainerElement.style.width = Math.min(window.innerWidth - 20, 336) + "px";
			M.adContainerElement.style.maxWidth = (window.innerWidth - 20) + "px";
			M.adContainerElement.style.height = Math.min(window.innerHeight, 280) + "px";
		} else {
		...
		try {
			...
			M.showAdModal({
				...
				"width": parseInt(M.adContainerElement.style.width), 
				"height": parseInt(M.adContainerElement.style.height)
		...
		module.prototype.onAdsManagerLoaded = function(adsManagerLoadedEvent){
			...
			faZepto.each(['LOADED', 'START', 'IMPRESSION', 'USER_CLOSE', 'SKIPPED', 'COMPLETE', 'ALL_ADS_COMPLETED'], function(key, value) {
		...
		module.prototype.onAdEvent = function(adEvent){
			...
			switch (adEvent.type) {
				case google.ima.AdEvent.Type.LOADED:
					self.spinner.hide();
					if (!ad.isLinear()) {
						self.modal.setDimensions(parseInt(M.adContainerElement.style.width), parseInt(M.adContainerElement.style.height));
						if (ad.getWidth()) {
							// self.modal.setDimensions(parseInt(M.adContainerElement.style.width), ad.getHeight() - 4);
						} else {
							// self.modal.setDimensions(640, ad.getHeight() - 4);
						}
					}
					switch (M.provider) {
						case 'adsense': 
							...
							break;
						case 'adx': 
							self.tracking.trackEvent("Ad event", "AdX", adEvent.type);
							break;
						case 'dfp': 
		...
		module.prototype.onAdError = function(adErrorEvent){
			switch (M.provider) {
				case 'adsense': 
					self.tracking.trackEvent("Ad event", "AdSense", 'error');
					self.tracking.trackEvent("Ad error event", "AdSense", adErrorEvent.getError());
					break;
				case 'adx': 
					self.tracking.trackEvent("Ad event", "AdX", 'error');
					self.tracking.trackEvent("Ad error event", "AdX", adErrorEvent.getError());
					break;
				case 'dfp': 
					self.tracking.trackEvent("Ad event", "DFP", 'error');
		...
		var myadTagUrl = "";
		var publisherId = M.publisherId; 
		var descriptionUrl = encodeURIComponent(self.config.ads.description_url);
		var age = "";
		var gender = "";
		var adtest = self.debug ? "on" : "";
		var channel = (M.provider == "adx") ? self.config.ads.adx_channels.join('+') : self.config.ads.adsense_channels.join('+');
		var adType = "image"; //video_text_image_flash
		var duration = 40000;
		var language = self.gametranslation.getUserLang();
		var slotname = (M.provider == "adx") ? self.config.aid : '';

		if (M.provider == "dfp") {
			// @see https://support.google.com/dfp_premium/answer/1068325?hl=de
			myadTagUrl = "http://pubads.g.doubleclick.net/gampad/ads?impl=s&gdfp_req=1&env=vp&output=vast&unviewed_position_start=1&url=[referrer_url]&correlator=[timestamp]&ciu_szs";
		...
		adsRequest.nonLinearAdSlotWidth = parseInt(M.adContainerElement.style.width);
		adsRequest.nonLinearAdSlotHeight = parseInt(M.adContainerElement.style.height);
	...
	module.prototype.showDFP = function(callback, force){
		var opts = {
			mode: "ad",
			width: M.dfp_ad_sizes[0],
			height: M.dfp_ad_sizes[1],
			closeBtnTimer: 10,
		...
		M.showAdModal(opts, faZepto("<div id='" + M.dfp_ad_div + "'></div>").get(0)).modal.setDimensions(opts.width, opts.height);

		/*googletag.cmd.push(function() {
			googletag.display(M.dfp_ad_div);
			googletag.pubads().refresh([M.dfp_ad_slot1]);

			self.tracking.trackEvent("Ad event", "DFP", "display");
		});*/
	...
	module.prototype.showAppWall = function(callback, force){
		...
		// show iframe
		var url = '/sda/appwall/' + window.famobi_gameID + '/' + window.famobi.config.aid;
		self.iframe.show(url, opts);
		self.modal.setHeader(self.translate("api.ad_modal_header2"));
		self.modal.setCloseCallback(opts.returnCallback);
		//self.modal.updateCloseBtn(true);
...
fg_api.prototype.init = function() {
	...
	faZepto(self.rootElement).on('faGame:loaded', function() {
		// change meta viewport content if its attribute 'data-original' exists
		var $metaviewport = document.querySelector("meta[name=viewport]"),
			ua = navigator.userAgent,
			metaViewPortSetting = $metaviewport.getAttribute("data-original");
